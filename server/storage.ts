...
export interface IStorage {
  ...
  listTimeEntries(userId?: string, startDate?: Date, endDate?: Date): Promise<TimeEntry[]>;
  ...
  listShifts(scheduleId?: string, startDate?: Date, endDate?: Date): Promise<Shift[]>;
  ...
  listKnowledgeArticles(category?: string, publishStatus?: string): Promise<KnowledgeArticle[]>;
  
  // Audit Logs
  createAuditLog(log: InsertAuditLog): Promise<AuditLog>;
  listAuditLogs(userId?: string, resourceType?: string, limit?: number): Promise<AuditLog[]>;
...
export class DbStorage implements IStorage {
  ...
  async updateUser(id: string, user: Partial<Omit<InsertUser, 'password'>> & { passwordHash?: string }): Promise<User | undefined> {
  ...
  async listTimeEntries(userId?: string, startDate?: Date, endDate?: Date): Promise<TimeEntry[]> {
  ...
  async listSchedules(status?: string): Promise<Schedule[]> {
    if (status) {
      return await db.select().from(schedules).where(eq(schedules.status, status)).orderBy(desc(schedules.startDate));
    }
  ...
  async updateShiftTemplate(id: string, template: Partial<InsertShiftTemplate>): Promise<ShiftTemplate | undefined> {
    const result = await db.update(shiftTemplates).set(template).where(eq(shiftTemplates.id, id)).returning();
    return result[0];
  }

  async deleteShiftTemplate(id: string): Promise<boolean> {
    const result = await db.delete(shiftTemplates).where(eq(shiftTemplates.id, id)).returning();
  ...
  async listShifts(scheduleId?: string, startDate?: Date, endDate?: Date): Promise<Shift[]> {
  ...
  async getShiftAssignment(id: string): Promise<ShiftAssignment | undefined> {
    const result = await db.select().from(shiftAssignments).where(eq(shiftAssignments.id, id));
    return result[0];
  }
  ...
  async updateShiftAssignment(id: string, assignment: Partial<InsertShiftAssignment>): Promise<ShiftAssignment | undefined> {
    const result = await db.update(shiftAssignments).set(assignment).where(eq(shiftAssignments.id, id)).returning();
    return result[0];
  }

  async deleteShiftAssignment(id: string): Promise<boolean> {
    const result = await db.delete(shiftAssignments).where(eq(shiftAssignments.id, id)).returning();
    return result.length > 0;
  }

  async listShiftAssignments(shiftId?: string, userId?: string): Promise<ShiftAssignment[]> {
  ...
  async updateKnowledgeArticle(id: string, article: Partial<InsertKnowledgeArticle>): Promise<KnowledgeArticle | undefined> {
    const result = await db.update(knowledgeArticles).set(article).where(eq(knowledgeArticles.id, id)).returning();
    return result[0];
  }
  ...
  async listKnowledgeArticles(category?: string, publishStatus?: string): Promise<KnowledgeArticle[]> {
    let query = db.select().from(knowledgeArticles);
    const conditions = [];
    
    if (category) conditions.push(eq(knowledgeArticles.category, category));
    if (publishStatus) conditions.push(eq(knowledgeArticles.publishStatus, publishStatus));
  ...
  async listAuditLogs(userId?: string, resourceType?: string, limit: number = 100): Promise<AuditLog[]> {
