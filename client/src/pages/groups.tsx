...
export default function Groups() {
  ...
  // Extract all unique programs from users' jobRates
  const autoProgramGroups = useMemo((): Group[] => {
    const programSet = new Set<string>();

    // Extract all programs from all users
    users.forEach(user => {
      if (user.jobRates && typeof user.jobRates === 'object') {
        ...
        programs.forEach(program => {
          if (program) { // Only add non-empty program names
    ...
    return Array.from(programSet).map(programName => ({
      id: `auto-program-${programName}`, // Prefix with 'auto-program-' to identify auto-generated groups
      name: programName,
      category: 'program',
      memberIds: [],
      assignmentIds: [],
  ...
  // Check if a group is auto-generated (read-only)
  const isAutoGeneratedGroup = (group: Group): boolean => {
    return group.id.startsWith('auto-program-');
  };
  ...
  const getMemberCount = (group: Group): number => {
    if (group.category === 'discipline') {
      ...
      // Users can have multiple programs (e.g., users can have multiple programs assigned)
      return users.filter(user => {
        ...
        const jobRatesObj = user.jobRates as Record<string, any>;
        const programs = Object.keys(jobRatesObj);
        return programs.includes(group.name);
      }).length;
    } else {
